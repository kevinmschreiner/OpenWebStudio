...
Namespace DataAccess.Factories
    Public Class ModuleActionFactory
        ...
        Public Sub PopulateActionMenu(ByRef Actions As DotNetNuke.Entities.Modules.Actions.ModuleActionCollection, ByVal modulePath As String, ByVal url As String)
            ...
            cM = pGI.Actions.Add(pM.GetNextActionID, "Clear Module Cache", "ClearModCache", Icon:="restore.gif", UseActionEvent:=True, Secure:=SecurityAccessLevel.Edit)
            cW = pGI.Actions.Add(pM.GetNextActionID, "Clear Web Cache", "ClearWebCache", Icon:="restore.gif", UseActionEvent:=True, Secure:=SecurityAccessLevel.Edit)
            cBreak1 = pGI.Actions.Add(pM.GetNextActionID, "~", "")
            cD = pGI.Actions.Add(pM.GetNextActionID, "Toggle Debugging", "ToggleDebugging", Icon:="rec.gif", UseActionEvent:=True, Secure:=SecurityAccessLevel.Edit)
            cR = pGI.Actions.Add(pM.GetNextActionID, "Toggle Redirects", "ToggleRedirects", Icon:="rev.gif", UseActionEvent:=True, Secure:=SecurityAccessLevel.Edit)
            cBreak2 = pGI.Actions.Add(pM.GetNextActionID, "~", "")
            cB = pGI.Actions.Add(pM.GetNextActionID, "Quick Builder", "Edit", "", "ratingplus.gif", Url:=Replace(url, "Edit", "Build"), UseActionEvent:=True, Secure:=SecurityAccessLevel.Edit, Visible:=True)
            pGI.Actions.Add(pM.GetNextActionID, "Repository", "Edit", "", "frew.gif", Url:=Replace(url, "Edit", "Repository"), UseActionEvent:=True, Secure:=SecurityAccessLevel.Edit, Visible:=True)
            pGI.Actions.Add(pM.GetNextActionID, "Log", "Edit", "", "icon_viewstats_16px.gif", Url:=Replace(url, "Edit", "Log"), UseActionEvent:=True, Secure:=SecurityAccessLevel.Edit, Visible:=True)
            ...
            Dim cV As DotNetNuke.Entities.Modules.Actions.ModuleAction = act.Add(pM.GetNextActionID, "View Options", "Edit", "", "Edit.gif", Url:=Replace(url, "Edit", "Edit"), UseActionEvent:=True, Secure:=SecurityAccessLevel.Edit, Visible:=True)
