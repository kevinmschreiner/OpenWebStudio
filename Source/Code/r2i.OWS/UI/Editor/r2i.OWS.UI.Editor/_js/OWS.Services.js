...
function OWSServicesClass() {
    ...
    this._method = function (url, params, single, callback) {
        if (this.loaded) {
            ...
            new OWSRequest(url, params, callback, name);
        }
    };
	this.request = function(url,params,single,callback) {
		if (typeof params == 'undefined') {
			if (typeof url.params=='undefined') 	{url.params=[]}
			if (typeof url.single=='undefined') 	{url.single=false}
			if (typeof url.callback=='undefined')	{url.callback=null}
			this._method(url.url,url.params,url.single,url.callback);
		} else {
			this._method(url,params,single,callback);
		}
	}
}
